...
export interface IProfileClient {
    ...
    updateProfile(id: number, command: UpdateUserProfileCommand | null | undefined): Observable<UserProfileDto>;
}
...
export class ProfileClient implements IProfileClient {
    ...
    protected processUpdateProfile(response: HttpResponseBase): Observable<UserProfileDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
